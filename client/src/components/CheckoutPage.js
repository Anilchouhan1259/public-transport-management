...
import { useFetchBusDetailQuery } from "../store/api/bussesApi";
...
function CheckoutPage({
  ...
  const { id } = useParams();
  console.log(id);
  const { data, isFetching } = useFetchBusDetailQuery(id);
  if (isFetching) {
    console.log(data);
  }
    ...
    const fetchFare = async () => {
      try {
        const response = await fetch(
          `/api/fare?fromLocation=${fromLocation}&toLocation=${toLocation}&scheduleId=${scheduleId}`
        );
  ...
  const renderPaymentResult = () => {
    if (paymentStatus === "success") {
      return (
        <p style={{ color: "green" }}>
          Payment Successful! Payment ID: DUMMY12345
        </p>
      );
